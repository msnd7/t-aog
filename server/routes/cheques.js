...
const { nowIso, chequeSerial, tafqit, toInt } = require('../util');
...
router.get('/catalog', (req, res) => {
  res.json({ catalog: chequeCatalog(), currency: getSettings().currency });
});

function hydrate(cheque) {
...
router.get('/', requireStaff, (req, res) => {
  ...
  const rows = db.prepare(`${CHEQUE_QUERY} ${where} ORDER BY c.id DESC LIMIT ?`).all(...params);
  res.json({ cheques: rows.map(hydrate) });
});

/** Cheques requested for printing, e.g. /api/cheques/print?ids=4,5,6 */
router.get('/print', requireStaff, (req, res) => {
  const ids = String(req.query.ids || '').split(',').map((v) => toInt(v)).filter(Boolean);
  if (!ids.length) return res.json({ cheques: [] });
  const placeholders = ids.map(() => '?').join(',');
  const rows = db.prepare(`${CHEQUE_QUERY} WHERE c.id IN (${placeholders}) ORDER BY c.id`).all(...ids);
  res.json({ cheques: rows.map(hydrate), settings: getSettings() });
});
...
router.post('/', requireStaff, (req, res) => {
  ...
  for (const studentId of studentIds) {
    const student = db.prepare("SELECT * FROM users WHERE id = ? AND role = 'student' AND active = 1").get(studentId);
    if (!student) continue;
    const halaqa = student.halaqa_id
      ? db.prepare('SELECT * FROM halaqat WHERE id = ?').get(student.halaqa_id) : null;
    ...
    db.prepare('UPDATE cheques SET serial = ? WHERE id = ?').run(serial, id);
  ...
  const rows = db.prepare(`${CHEQUE_QUERY} WHERE c.id IN (${placeholders}) ORDER BY c.id`).all(...created);
  res.status(201).json({ ids: created, cheques: rows.map(hydrate), total });
});

router.post('/printed', requireStaff, (req, res) => {
  ...
  const stmt = db.prepare('UPDATE cheques SET printed_at = ? WHERE id = ?');
  const at = nowIso();
...
router.delete('/:id', requireStaff, (req, res) => {
  ...
  const cheque = db.prepare('SELECT * FROM cheques WHERE id = ?').get(id);
  if (!cheque) return res.status(404).json({ error: 'الشيك غير موجود' });
  db.prepare('DELETE FROM point_entries WHERE cheque_id = ?').run(id);
