...
Vercel يشغّل المنصة كدالة بلا خادم، وقرصه **للقراءة فقط ويُمحى مع كل نشر**. لذلك:

- **لا بد من قاعدة بيانات بعيدة** وإلا فكل البيانات (الطلاب والنقاط والشيكات) تُفقد خلال دقائق.
  المنصة تدعم قواعد **libSQL/Turso** وهي SQLite نفسها لكن على الشبكة، وفيها خطة مجانية.
- الصور المرفوعة (صور الطلاب والجوائز والشعار) تُحفظ تلقائياً **داخل قاعدة البيانات**
  عندما لا يوجد قرص دائم، فلا حاجة لخدمة تخزين إضافية.

### ١) أنشئ قاعدة البيانات

من [turso.tech](https://turso.tech) أنشئ حساباً ثم قاعدة جديدة، أو من الطرفية:
...
5. أضف متغيرات البيئة (**Environment Variables**) لكل البيئات:
...
> **إن لم تضبط `TURSO_DATABASE_URL`** فالمنصة تعمل على Vercel لكن قاعدة بياناتها مؤقتة في
> ذاكرة الخادم: تصلح للتجربة والعرض فقط، وتُفقد البيانات مع كل نشر أو خمول. لا تستخدمها للمجمع.

> **نسخة احتياطية**: `turso db shell riyad-quran ".dump" > backup-$(date +%F).sql`
