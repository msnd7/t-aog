...
1. **قرص دائم** لمجلد `data/` (فيه `app.db` وصور الطلاب والجوائز) — بدونه تُفقد البيانات مع كل تحديث.
   وإن كانت الاستضافة بلا قرص دائم (مثل Vercel) فاربطها بقاعدة libSQL بعيدة كما في قسم Vercel أدناه.
2. **HTTPS** — لازم لتثبيت المنصة كتطبيق (PWA) ولحماية جلسات الدخول، ومعه `COOKIE_SECURE=1`.

المستودع جاهز للنشر: `Dockerfile` و`fly.toml` و`render.yaml` و`vercel.json` وملفات `deploy/` للخادم الخاص،
...
## الخيار الثاني: Vercel — ربط بالمستودع، مع قاعدة بيانات خارجية

Vercel يشغّل المنصة كدالة بلا خادم، وقرصه **للقراءة فقط ويُمحى مع كل نشر**. لذلك **لا بد من
قاعدة بيانات بعيدة** وإلا فكل البيانات (الطلاب والنقاط والشيكات) تُفقد خلال دقائق. المنصة
تكتشف قاعدة البيانات تلقائياً من متغيرات البيئة، بالترتيب التالي:

1. **Postgres** — عبر `POSTGRES_URL` أو `DATABASE_URL`. هذا هو تكامل **Neon** الذي يضيفه Vercel
   تلقائياً عند ربط Storage ← Postgres بالمشروع (Marketplace)، فلا حاجة لحساب أو إعداد يدوي؛
   يكفي ربط قاعدة Postgres بالمشروع من تبويب **Storage** في Vercel وستظهر المتغيرات وحدها.
2. **libSQL/Turso** — عبر `TURSO_DATABASE_URL` — وهي SQLite نفسها لكن على الشبكة، بديل بحساب
   مجاني منفصل إن فضّلت عدم استخدام Postgres.

الصور المرفوعة (صور الطلاب والجوائز والشعار) تُحفظ تلقائياً **داخل قاعدة البيانات** عندما لا
يوجد قرص دائم، فلا حاجة لخدمة تخزين إضافية أياً كان الخيار.

### ١) أنشئ قاعدة البيانات

**الأسهل (Postgres عبر Neon):** من لوحة المشروع في Vercel افتح تبويب **Storage** ← **Create
Database** ← اختر **Postgres (by Neon)** واربطها بالمشروع. تُضاف متغيرات `POSTGRES_URL` وما
شابهها تلقائياً لكل البيئات (Production/Preview/Development) دون أي خطوة إضافية.

**أو libSQL/Turso:** من [turso.tech](https://turso.tech) أنشئ حساباً ثم قاعدة جديدة، أو من الطرفية:

```bash
curl -sSfL https://get.tur.so/install.sh | bash
turso auth signup
turso db create riyad-quran
turso db show riyad-quran --url          # ‏libsql://riyad-quran-xxxx.turso.io
turso db tokens create riyad-quran       # الرمز السري
```

### ٢) اربط المستودع بـ Vercel

1. ادخل [vercel.com/new](https://vercel.com/new) وسجّل الدخول بحساب GitHub.
2. اختر مستودع `msnd7/t-aog` ثم **Import**.
3. **Framework Preset**: `Other` — واترك أوامر البناء فارغة (المشروع بلا أدوات بناء).
4. تأكد من **Node.js Version = 22.x** فأحدث (Settings ← General).
5. إن استخدمت Postgres عبر تبويب Storage فمتغيراته مضبوطة بالفعل، ولا يبقى إلا:

   | المتغير | القيمة |
   | --- | --- |
   | `ADMIN_PHONE` | رقم جوال المدير الأول، مثل `0512345678` |
   | `TZ` | `Asia/Riyadh` |

   وإن اخترت libSQL/Turso بدلاً من ذلك أضف أيضاً:

   | المتغير | القيمة |
   | --- | --- |
   | `TURSO_DATABASE_URL` | ‏`libsql://riyad-quran-xxxx.turso.io` |
   | `TURSO_AUTH_TOKEN` | الرمز السري من الخطوة السابقة |

6. اضغط **Deploy**، ثم افتح الرابط `https://<اسم-المشروع>.vercel.app`.

ملف `vercel.json` في المستودع يوجّه الطلبات، و`api/index.js` هو مدخل التشغيل، وهما جاهزان
فلا تحتاج إلى ضبط شيء إضافي. بعد الربط يُنشر كل دفع (push) إلى `main` تلقائياً على النطاق
الأساسي، وكل فرع آخر أو Pull Request يحصل على رابط معاينة مستقل.

> **إن لم تضبط أياً من `POSTGRES_URL`/`DATABASE_URL`/`TURSO_DATABASE_URL`** فالمنصة تعمل على
> Vercel لكن قاعدة بياناتها مؤقتة في ملف محلي يُمحى مع كل نشر أو خمول: تصلح للتجربة والعرض
> فقط، ولا تُستخدم للمجمع.

> **نسخة احتياطية**: لقاعدة Postgres استخدم `pg_dump "$POSTGRES_URL" > backup-$(date +%F).sql`
> (من تبويب Storage في Vercel يمكن أيضاً فتح لوحة Neon مباشرة لأخذ نسخة). لقاعدة Turso:
> `turso db shell riyad-quran ".dump" > backup-$(date +%F).sql`

## الخيار الثالث: Render — ربط مباشر بالمستودع بضغطة
...
## الخيار الرابع: خادم خاص (VPS) — الأقوى للاستخدام اليومي في المجمع
